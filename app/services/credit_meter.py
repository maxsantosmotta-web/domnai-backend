...
# Valores por 1 milhão de tokens. Podem ser sobrescritos por variáveis de ambiente.
...
def charge_usage(user_id: str, result: MeteredBrainResult) -> dict:
    ...
    with session_scope() as db:
        ...
        if available < usage["credits"]:
            # A chamada já foi processada pelo provedor. Bloqueia novas chamadas e
            # consome o saldo restante sem permitir saldo negativo.
            charged = available
        else:
            charged = usage["credits"]
