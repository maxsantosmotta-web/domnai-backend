...
new_function = '''def _simple_conversation_response(message: str, attachments: list[dict], operation: str | None = None) -> str | None:
    ...
    normalized = " ".join(_normalized_text(message).replace("?", "").replace("!", "").replace(",", " ").split())
    if not normalized or len(normalized) > 120:
        return None

    greeting_prefixes = (
        "oi", "ola", "bom dia", "boa tarde", "boa noite", "e ai",
        "fala chat", "oi chat", "ola chat",
    )
    wellbeing_markers = (
        "tudo bem", "como voce esta", "como vai voce", "como vai", "como esta hoje",
    )
    thanks_messages = {
        "obrigado", "obrigada", "muito obrigado", "muito obrigada", "valeu", "agradecido", "agradecida",
    }
    ...
    confusion_markers = (
        "nao entendi", "não entendi", "explica melhor", "pode explicar", "ficou confuso",
    )
    reaction_markers = (
    ...
    if any(normalized == prefix or normalized.startswith(f"{prefix} ") for prefix in greeting_prefixes):
        return "Oi! Como posso ajudar você hoje?"
    if any(marker in normalized for marker in wellbeing_markers):
        return "Estou bem e pronto para ajudar. E você, como está?"
    if normalized in thanks_messages:
...
addition = (
    ...
    'pedir explicação ou fizer uma pergunta natural, responda a isso primeiro. Não ignore a mensagem, não repita a '
    'pergunta operacional anterior e não pareça um formulário. Mantenha a operação disponível para continuidade natural.\n'
